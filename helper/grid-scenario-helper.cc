...
GridScenarioHelper::CreateScenario ()
{
  ...
  if (m_ut.GetN () > 0)
    {
      ...
      for (uint32_t i = 0; i < utN; ++i)
        {
          ...
          if(m_fixedUser)
          {
            pos.x += m_x->GetMax ()*(1+i);
            pos.y += m_y->GetMax ()*(1+i);
          }
          else
          {
            pos.x += m_x->GetValue ();
	  ...
	  std::cout<<"UE position: " <<pos<<std::endl;
  ...
  m_ut.Get (0)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (0, m_speed, 0)); // move UE1 along the y axis
}
