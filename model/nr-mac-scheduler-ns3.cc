...
NrMacSchedulerNs3::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrMacSchedulerNs3")
    ...
    .AddTraceSource ("MacSrGrant",
                     "Transmit scheduling request grant message.",
                     MakeTraceSourceAccessor (&NrMacSchedulerNs3::m_macSRGrant),
                     "ns3::Time::TracedCallback")
    .AddTraceSource ("MacBsrGrant",
                     "Transmit buffer status report grant message.",
                     MakeTraceSourceAccessor (&NrMacSchedulerNs3::m_macBSRGrant),
                     "ns3::Time::TracedCallback")
            ...
            if (allocation.m_symStart == symStart)
              {
                ...
                //std::cout<<"Num RB per RBG: "<< m_macSchedSapUser->GetNumRbPerRbg ()<<std::endl;
  ...
  for (auto harqIt = harq->Begin (); harqIt != harq->End (); ++harqIt)
    {
      ...
      if (process.m_timer <m_macSchedSapUser->GetNumHarqProcess())
        {
          ++process.m_timer;
          NS_LOG_INFO (Simulator::Now().GetSeconds()<<" Updated process for UE " << rnti << " number " <<
      ...
      for (const auto &ue : beam.second)
        {
          ...
          std::cout<<" UE" << dci->m_rnti <<
                        " gets UL symbols " << static_cast<uint32_t> (dci->m_symStart) <<
                        "-" << static_cast<uint32_t> (dci->m_symStart + dci->m_numSym) <<
                        " tbs " << dci->m_tbSize <<
                        " mcs " << static_cast<uint32_t> (dci->m_mcs) <<
                        " harqId " << static_cast<uint32_t> (id) <<
                        " rv " << static_cast<uint32_t> (dci->m_rv)<<std::endl;
  ...
  for (const auto & v : rntiList)
    {
      for (auto & ulLcg : NrMacSchedulerUeInfo::GetUlLCG (m_ueMap.at (v)))
        {
          NS_LOG_INFO ("Assigning 12 bytes to UE " << v << " because of a SR");
          m_macSRGrant(Simulator::Now());
                                   ...
                                   const std::vector <UlHarqInfo> &ulHarqFeedback)
{
  ...
  //std::cout<<Simulator::Now()<<" UL scheduled "<<ulSlot.m_slotAllocInfo<<std::endl;
