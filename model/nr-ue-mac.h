...
private:
  ...
  // 210407 Goodsol: SR configuration
  uint16_t m_srPeriodicity;
  Time m_periodWaitTime;
  ...
  TracedCallback<uint32_t> m_macNotify;
  TracedCallback<uint32_t> m_macSR;
  TracedCallback<uint32_t> m_macBSR;
  TracedCallback<Time> m_macTx;
  TracedCallback<Time> m_macRxUlDci;
};
