...
NrUeMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrUeMac")
    ...
    .AddAttribute ("SRperiodicity",
                   "Scheduling request periodicity",
                    UintegerValue (10),
                    MakeUintegerAccessor (&NrUeMac::m_srPeriodicity),
                    MakeUintegerChecker<uint8_t> ())
    ...
    .AddTraceSource ("MacTxNotify",
                     "Notify RLC buffer time.",
                     MakeTraceSourceAccessor (&NrUeMac::m_macNotify),
                     "ns3::Time::TracedCallback")
    .AddTraceSource ("MacTxSr",
                     "Scheduling request poll time.",
                     MakeTraceSourceAccessor (&NrUeMac::m_macSR),
                     "ns3::Time::TracedCallback")
    .AddTraceSource ("MacTxBsr",
                     "Buffer Status report poll time.",
                     MakeTraceSourceAccessor (&NrUeMac::m_macBSR),
                     "ns3::Time::TracedCallback")
    .AddTraceSource ("MacTxData",
                     "Uplink MAC packet Traces.",
                     MakeTraceSourceAccessor (&NrUeMac::m_macTx),
                     "ns3::Time::TracedCallback")
    .AddTraceSource ("MacRxUlDci",
                     "Uplink MAC packet Traces.",
                     MakeTraceSourceAccessor (&NrUeMac::m_macRxUlDci),
                     "ns3::Time::TracedCallback")
  ;
  return tid;
}
...
NrUeMac::DoTransmitPdu (LteMacSapProvider::TransmitPduParameters params)
{
  ...
  //std::cout<<Simulator::Now().GetSeconds()<<" PDU sent "<<params.pdu->GetSize ()<<" "<<m_ulDci->m_tbSize<< std::endl;
...
NrUeMac::DoReportBufferStatus (LteMacSapProvider::ReportBufferStatusParameters params)
{
  ...
  NS_LOG_INFO ("Received BSR for LC Id" << m_rnti<<" "<< static_cast<uint32_t>(params.lcid) << GetTotalBufSize());
  ...
  if (m_srState == INACTIVE)
    {
      m_macNotify(GetTotalBufSize ());
      NS_LOG_INFO ("INACTIVE -> TO_SEND, bufSize " << GetTotalBufSize ());
      //std::cout<<Simulator::Now().GetSeconds()<<" INACTIVE -> TO_SEND, bufSize " << GetTotalBufSize ()<<std::endl;
      m_srState = TO_SEND;
    }
}
  ...
  for (it = m_ulBsrReceived.begin (); it != m_ulBsrReceived.end (); it++)
    {
      ...
      queue.at (lcg) += ((*it).second.txQueueSize + (*it).second.retxQueueSize + (*it).second.statusPduSize)*0.7;
    }

  NS_LOG_INFO ("Sending BSR with this info for the LCG: " << queue.at (0) << " " <<
               queue.at (1) << " " << queue.at(2) << " " << queue.at(3));
  //std::cout<<Simulator::Now().GetSeconds()<<" "<<m_ulDciTotalUsed<<" "<<queue.at (0) << " " << 
  //             queue.at (1) << " " << queue.at(2) << " " << queue.at(3)<<std::endl;
  ...
  for (uint16_t i = 0; i < m_miUlHarqProcessesPacketTimer.size (); i++)
    {
      if (m_miUlHarqProcessesPacketTimer.at (i) == 0 && m_miUlHarqProcessesPacket.at (i).m_pktBurst)
        {
	  //std::cout<<"HI"<<m_rnti<<" "<<m_miUlHarqProcessesPacket.at (i).m_pktBurst->GetSize ()<<std::endl;
...
NrUeMac::DoSlotIndication (const SfnSf &sfn)
{
  ...
  if (m_srState == TO_SEND)
    {
      if((sfn.GetFrame()*10+sfn.GetSubframe())%m_srPeriodicity==(2%m_srPeriodicity))
      {
        NS_LOG_INFO ("Sending SR to PHY in slot " << sfn);
        std::cout<<Simulator::Now().GetSeconds()<<" UE: "<<m_rnti<<" Sending SR to PHY in slot " << sfn << std::endl;
        SendSR ();
        m_srState = ACTIVE;
      }
    }
...
NrUeMac::SendSR () const
{
  ...
  //std::cout<<Simulator::Now().GetSeconds()<<" SendSR from UE"<<m_rnti<<std::endl;
...
NrUeMac::ProcessUlDci (const Ptr<NrUlDciMessage> &dciMsg)
{
  ...
  //std::cout<<Simulator::Now().GetSeconds()<<" RNTI: "<< m_rnti<<" TBS: "<<m_ulDci->m_tbSize<<" queue: "<<GetTotalBufSize()<<std::endl;

  m_macRxUlDci(Simulator::Now());
...
NrUeMac::DoReceiveControlMessage  (Ptr<NrControlMessage> msg)
{
  ...
  switch (msg->GetMessageType ())
    ...
    case (NrControlMessage::UL_DCI):
      {
        //std::cout<<Simulator::Now().GetSeconds()<<" UCI received"<<std::endl;
