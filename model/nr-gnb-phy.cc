...
NrGnbPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrGnbPhy")
    ...
    .AddAttribute ("TxPower",
                   ...
                   DoubleValue (30.0), //modified by Goodsol Lee.. why only 4.0dBm tx power for gNB??
    ...
    .AddTraceSource ("PhyRxSr",
                     "Phy scheduling request transmit time.",
                     MakeTraceSourceAccessor (&NrGnbPhy::m_phyRxSR),
                     "ns3::Time::TracedCallback")
    .AddTraceSource ("PhyRxData",
                     "Phy scheduling request transmit time.",
                     MakeTraceSourceAccessor (&NrGnbPhy::m_phyRxData),
                     "ns3::Time::TracedCallback")
    .AddTraceSource ("PhyTxUlDci",
                     "Phy scheduling request transmit time.",
                     MakeTraceSourceAccessor (&NrGnbPhy::m_phyUlDci),
                     "ns3::Time::TracedCallback")
    ;
  return tid;
      ...
      for (auto ctrlIt = m_ctrlMsgs.begin (); ctrlIt != m_ctrlMsgs.end (); ++ctrlIt)
        {
          ...
          {
            //std::cout<<Simulator::Now()<<" UL_DCI"<<std::endl;
...
NrGnbPhy::UlData(const std::shared_ptr<DciInfoElementTdma> &dci)
{
  ...
  NS_LOG_INFO ("GNB RXing UL DATA frame " << m_currentSlot <<
                ...
                " end " << Simulator::Now () + varTtiPeriod);
  /*
  std::cout<<Simulator::Now().GetSeconds()<<"GNB RXing UL DATA frame " << m_currentSlot <<
                " symbols "  << static_cast<uint32_t> (dci->m_symStart) <<
                "-" << static_cast<uint32_t> (dci->m_symStart + dci->m_numSym - 1) <<
                " start " << Simulator::Now () <<
  ...
  if (msg->GetMessageType () == NrControlMessage::DL_CQI)
  ...
  else
    {
      ...
      if (msg->GetMessageType () == NrControlMessage::SR)
        {
          //std::cout<<"SR Phy RX"<<std::endl;
          m_phyRxSR(Simulator::Now());
        }
