...
NrUePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrUePhy")
    ...
    .AddTraceSource ("PhyTxSr",
                     "Phy scheduling request transmit time.",
                     MakeTraceSourceAccessor (&NrUePhy::m_phySR),
                     "ns3::Time::TracedCallback")
    .AddTraceSource ("PhyTxData",
                     "Phy data Tx time.",
                     MakeTraceSourceAccessor (&NrUePhy::m_phyTx),
                     "ns3::Time::TracedCallback")
    .AddTraceSource ("PhyRxUlDci",
                     "Phy receive UL-DCI.",
                     MakeTraceSourceAccessor (&NrUePhy::m_phyRxUlDci),
                     "ns3::Time::TracedCallback")
      ;
  return tid;
}
...
NrUePhy::ComputeAvgSinr (const SpectrumValue &sinr)
{
  ...
  //std::cout<<m_rnti<<" "<<Simulator::Now().GetSeconds()<<" SINR: "<<10*log10(avrgSinr)<<std::endl;
  ...
  for (const auto & msg : m_ctrlMsgs)
    {
      ...
      //std::cout<<"PHY message type: "<<msg->GetMessageType ()<<std::endl;
...
NrUePhy::UlData(const std::shared_ptr<DciInfoElementTdma> &dci)
{
  ...
  if (m_enableUplinkPowerControl)
    {
      std::cout<<"RBG bits: "<<(FromRBGBitmaskToRBAssignment(dci->m_rbgBitmask)).size()<<std::endl;
                           ...
                           const Time &duration)
{
  ...
  //std::cout<<"PHY tx trace: "<< pb->GetNPackets () << duration<<std::endl;
  m_phyTx(Simulator::Now());
